...
namespace PSIRTApp.Models
{
    public class SwisClient : ISwisClient
    {
        ...
        private async Task<JToken> SwisCallAsync(Func<HttpClient, Task<HttpResponseMessage>> doRequest)
        {
            ...
            //WebRequest aa = WebRequest.Create(string.Format("https://{0}:17778/SolarWinds/InformationService/v3/Json/", _hostname));
            //aa.Credentials = new NetworkCredential(_username, _password);
            //aa.PreAuthenticate = true;

            var handleCredential = new HttpClientHandler
            {
                 Credentials = new NetworkCredential(_username, _password),
                PreAuthenticate = true,
            ...
            using (var client = new HttpClient(handleCredential))
            {
